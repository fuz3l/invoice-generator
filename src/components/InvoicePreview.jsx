...
const InvoicePreview = ({ invoiceData, onGeneratePDF, template = "modern" }) => {
  ...
  const ModernTemplate = () => (
    <div
      id="invoice-template"
      className="p-4 sm:p-6 lg:p-8 rounded-lg shadow-lg max-w-4xl mx-auto"
      style={{ background: '#fff', color: '#222' }}
    >
      {/* Header */}
      <div style={{ borderBottom: '2px solid #e5e7eb', paddingBottom: 16, marginBottom: 16, sm: { paddingBottom: 24, marginBottom: 24 } }}>
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
          <div>
            {userData?.logoBase64 && (
              <img src={userData.logoBase64} alt="Business Logo" className="h-8 w-8 sm:h-12 sm:w-12 object-contain rounded mb-2 border" style={{maxWidth: 64}} />
            )}
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold mb-2" style={{ color: '#0f172a' }}>
              {userData?.businessName || "Your Business Name"}
            </h1>
            <p className="mb-1 text-sm sm:text-base" style={{ color: '#334155' }}>{userData?.businessAddress || "Business Address"}</p>
            <p className="mb-1 text-sm sm:text-base" style={{ color: '#334155' }}>GSTIN: {userData?.gstin || "GSTIN Number"}</p>
            <p className="text-sm sm:text-base" style={{ color: '#334155' }}>Owner: {userData?.businessOwner || "Business Owner"}</p>
          </div>
          <div style={{ textAlign: 'left', sm: { textAlign: 'right' } }}>
            <h2 className="text-xl sm:text-2xl font-bold mb-2" style={{ color: '#2563eb' }}>INVOICE</h2>
            <p className="mb-1 text-sm sm:text-base" style={{ color: '#334155' }}>Invoice #: {invoiceNumber}</p>
            <p className="mb-1 text-sm sm:text-base" style={{ color: '#334155' }}>Date: {currentDate}</p>
          </div>
        </div>
      </div>
    ...
    return (
      <>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-8 mb-6 sm:mb-8">
          <div>
            <h3 className="text-base sm:text-lg font-semibold mb-2 sm:mb-3" style={{ color: accent, fontFamily }}>Bill To:</h3>
            <p className="font-medium mb-1 text-sm sm:text-base" style={{ color: '#222', fontFamily }}>
              {invoiceData.customerName || "Customer Name"}
            </p>
            {invoiceData.customerEmail && (
              <p className="mb-1 text-sm sm:text-base" style={{ color: '#666', fontFamily }}>{invoiceData.customerEmail}</p>
            )}
            {invoiceData.customerPhone && (
              <p className="mb-1 text-sm sm:text-base" style={{ color: '#666', fontFamily }}>{invoiceData.customerPhone}</p>
            )}
            {invoiceData.customerAddress && (
              <p className="text-sm sm:text-base" style={{ color: '#666', fontFamily }}>{invoiceData.customerAddress}</p>
            )}
          </div>
          <div>
            <h3 className="text-base sm:text-lg font-semibold mb-2 sm:mb-3" style={{ color: accent, fontFamily }}>From:</h3>
            <p className="font-medium mb-1 text-sm sm:text-base" style={{ color: '#222', fontFamily }}>
              {userData?.businessName || "Your Business Name"}
            </p>
            <p className="mb-1 text-sm sm:text-base" style={{ color: '#666', fontFamily }}>{userData?.businessAddress || "Business Address"}</p>
            <p className="mb-1 text-sm sm:text-base" style={{ color: '#666', fontFamily }}>GSTIN: {userData?.gstin || "GSTIN Number"}</p>
            <p className="text-sm sm:text-base" style={{ color: '#666', fontFamily }}>Contact: {userData?.mobile || "Mobile Number"}</p>
          </div>
        </div>
        {/* Items Table */}
        <div className="mb-6 sm:mb-8 overflow-x-auto">
          <table className="w-full border-collapse min-w-full" style={tableStyle}>
            <thead>
              <tr>
                <th style={{ ...thStyle, padding: '8px 12px sm:12px 16px', textAlign: 'left', fontFamily, fontSize: '14px' }}>Item Description</th>
                <th style={{ ...thStyle, padding: '8px 12px sm:12px 16px', textAlign: 'center', fontFamily, fontSize: '14px' }}>Quantity</th>
                <th style={{ ...thStyle, padding: '8px 12px sm:12px 16px', textAlign: 'right', fontFamily, fontSize: '14px' }}>Unit Price (₹)</th>
                <th style={{ ...thStyle, padding: '8px 12px sm:12px 16px', textAlign: 'right', fontFamily, fontSize: '14px' }}>Amount (₹)</th>
              </tr>
            </thead>
            <tbody>
              {invoiceData.items?.map((item, index) => (
                <tr key={index} style={{ background: index % 2 === 0 ? '#fff' : '#f8fafc', fontFamily }}>
                  <td style={{ ...tdStyle, padding: '8px 12px sm:12px 16px' }}>
                    <div>
                      <p className="font-medium text-sm sm:text-base" style={{ color: '#222', fontFamily }}>{item.productName || "Product Name"}</p>
                      {item.description && (
                        <p className="text-xs sm:text-sm" style={{ color: '#64748b', fontFamily }}>{item.description}</p>
                      )}
                    </div>
                  </td>
                  <td style={{ ...tdStyle, padding: '8px 12px sm:12px 16px', textAlign: 'center', color: '#222', fontFamily, fontSize: '14px' }}>
                    {item.quantity || 0}
                  </td>
                  <td style={{ ...tdStyle, padding: '8px 12px sm:12px 16px', textAlign: 'right', color: '#222', fontFamily, fontSize: '14px' }}>
                    ₹{item.price?.toFixed(2) || "0.00"}
                  </td>
                  <td style={{ ...tdStyle, padding: '8px 12px sm:12px 16px', textAlign: 'right', color: '#222', fontWeight: 500, fontFamily, fontSize: '14px' }}>
          ...
          <div style={{ width: '100%', maxWidth: 320 }}>
            <div style={{ borderTop: '1px solid #e5e7eb', paddingTop: 16 }}>
              <div className="flex justify-between mb-2">
                <span className="text-sm sm:text-base" style={{ color: '#666', fontFamily }}>Subtotal:</span>
                <span className="text-sm sm:text-base font-medium" style={{ fontFamily }}>₹{subtotal.toFixed(2)}</span>
              </div>
              <div className="flex justify-between mb-2">
                <span className="text-sm sm:text-base" style={{ color: '#666', fontFamily }}>GST{gstPercent ? ` (${gstPercent}%)` : ''}:</span>
                <span className="text-sm sm:text-base font-medium" style={{ fontFamily }}>₹{gstAmount.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-base sm:text-lg font-bold" style={{ color: totalColor, borderTop: '1px solid #e5e7eb', paddingTop: 8, fontFamily }}>
                <span>Total:</span>
        ...
        {/* Footer */}
        <div style={{ marginTop: 32, sm: { marginTop: 48 }, paddingTop: 16, sm: { paddingTop: 24 }, borderTop: '1px solid #e5e7eb', fontFamily }}>
          <div style={{ textAlign: 'center', color: '#64748b', fontFamily }}>
            <p className="mb-2 text-sm sm:text-base">Thank you for your business!</p>
  ...
  // --- Render ---
  return (
    <div className="p-4 sm:p-6 rounded-lg shadow-md" style={{ background: template === 'colorful' ? '#fff7ed' : '#e0f2fe' }}>
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-6 gap-4">
        <h2 className="text-xl sm:text-2xl font-bold" style={{ color: '#0f172a' }}>Invoice Preview</h2>
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
          ...
          <button
            ...
            className="px-4 py-2 rounded-md font-medium flex items-center justify-center gap-2 transition-colors text-sm sm:text-base"
            style={{ background: '#25D366', color: '#fff' }}
            title="Share to WhatsApp"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 24 24" className="sm:w-5 sm:h-5">
